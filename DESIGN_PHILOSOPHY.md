...
This document is the contributor guide for humans and AI agents working on `easytable`.
...
4. No zebra striping on model statistics (`N`, `R sq.`, `Adj. R sq.`, `AIC`) or control indicators.
5. No horizontal rules between individual coefficient rows.
6. Exactly one divider line between coefficient block and model-stat block.
7. Control indicators from `control.var` belong to the model-stat block.
...
3. documentation explains both the user path and architecture limits clearly.
